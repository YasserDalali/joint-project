...
SELECT 'Seed User ' + CAST(n AS NVARCHAR(10)),
       'u' + RIGHT(REPLICATE('0', 6) + CAST(n AS NVARCHAR(6)), 6) + '@seed.finrisk.local',
...
SELECT 'SEED' + RIGHT(REPLICATE('0', 5) + CAST(n AS NVARCHAR(5)), 5),
       'Seed Instrument ' + CAST(n AS NVARCHAR(10)),
       CASE ((n - 1) % 4)
           WHEN 0 THEN 'STOCK'
           WHEN 1 THEN 'ETF'
           WHEN 2 THEN 'BOND'
...
SELECT id,
       CASE WHEN id % 3 = 0 THEN 'Technology' WHEN id % 3 = 1 THEN 'Healthcare' ELSE 'Financials' END,
       CASE WHEN id % 2 = 0 THEN 'NASDAQ' ELSE 'NYSE' END
FROM dbo.assets
WHERE asset_type = 'STOCK';

INSERT INTO dbo.asset_details_etf (asset_id, issuer, expense_ratio)
SELECT id,
       'Seed Issuer ' + CAST((id % 50) + 1 AS NVARCHAR(10)),
...
SELECT id,
       ...
       'Seed Sovereign ' + CAST((id % 20) + 1 AS NVARCHAR(10))
FROM dbo.assets
WHERE asset_type = 'BOND';

INSERT INTO dbo.asset_details_crypto (asset_id, blockchain)
SELECT id,
       CASE WHEN id % 3 = 0 THEN 'Ethereum' WHEN id % 3 = 1 THEN 'Bitcoin' ELSE 'Solana' END
FROM dbo.assets
...
VALUES (@demo_account, @aid1, 'BUY', 100, @p1, SYSUTCDATETIME()),
       (@demo_account, @aid2, 'BUY', 100, @p2, SYSUTCDATETIME()),
       (@demo_account, @aid3, 'BUY', 100, @p3, SYSUTCDATETIME());
